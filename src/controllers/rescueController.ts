...
const enrichCaseRecordWithReporterAndStray = async (formatted: any, caseIdOrId: string = "", requestOrHistoryUser: string = "") => {
  try {
    ...
    // Enrich Assigned Rescuer details (name, avatar, phone)
    const Rescuer = require("../models/Rescuer");
    ...
    if (rName || rAvatar) {
      ...
      if (formatted.rescuer) {
        ...
        formatted.rescuer = {
          ...
          phone: rPhone,
          avatar: rAvatar,
          location: rescuerDoc.location || formatted.location,
        };
      }
    }
  } catch (err: any) {
    console.error("[RESCUE] enrichCaseRecord error:", err.message);
  }
  return formatted;
};
...
exports.cancelRescueRequest = catchAsync(async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  ...
  // Notify the rescuer immediately via Socket.IO
  try {
...
exports.getRescueById = catchAsync(async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  ...
  const strayQuery: any[] = [{ caseId: String(id) }];
  if (isObjectId) strayQuery.push({ _id: String(id) });
  const strayDoc = await StrayReport.findOne({ $or: strayQuery });

  if (strayDoc) {
    let formatted = formatCaseRecord({ request: strayDoc });
    formatted = await enrichCaseRecordWithReporterAndStray(formatted, strayDoc.caseId || String(id), strayDoc.reporterUserId);

    res.json({
      ...formatted,
      reporterLocation: formatted.reporter.location || formatted.location,
      rescuerLocation: formatted.rescuer?.location || null,
      distanceKm: formatted.distanceKm ?? 0,
      etaMinutes: formatted.etaMinutes ?? 5,
      lastUpdatedAt: getIsoString(strayDoc.updatedAt || strayDoc.createdAt),
    });
    return;
  }
...
exports.checkRequestStatus = catchAsync(async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  ...
  res.json({
    ...
    caseStatus: formatted.status || request.status,
    requestId: String(request._id),
